...
class Message1Widget extends StatefulWidget {
  const Message1Widget({Key? key, required this.D}) : super(key: key);
...
class _Message1WidgetState extends State<Message1Widget> {
  ...
  Future<List<List<dynamic>>> getMessage() async {
    ...
    final response = await http.post(url, body: {
      "contractAddress": widget.D["contract"],
    ...
    if (response.statusCode == 200) {
      ...
      for (var message in data["message"]) {
        ...
        if ((message[0] == "3" && message[1] == "1") || (message[0] == "1" && message[1] == "3")) {
